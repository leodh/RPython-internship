...
class Withk(Contk):
    ...
    def apply(self, expr, env, val):
        env2 = self.env
        promote(env2)
        newEnv = env2.add_attribute(self.name, val)
        return self.expr, newEnv, self.cont, val
...
class Appk(Contk):
    ...
    def apply(self, expr, env, val):
        env2 = EMPTY_MAP
        promote(env2)
        newEnv = env2.add_attribute(self.arg, val)
        return self.expr, newEnv, self.cont, val
...
def Interpk(expr, funDict, env):
    ...
    while 1:
        ...
        #
            ...
            promote(env)
            val = env.getvalue(expr.name)
            if val == None:
                print "Free variable : %s" % expr.name
            ex, en, co, va = cont.apply(expr,env,val) 
            expr = ex
            env = en
            cont = co
